...
def handler(event, context):
    s3client.download_file(event['bucket'], event['key'], "/tmp/payload.json")
    with open("/tmp/payload.json", 'r') as map_file:
    ...
    for contract in payload:
        ...
        cnab['venal_value'] = table['venal_value'].apply(
            lambda x: x.replace(".", "").zfill(11) if x != "0.00" else "  DELETE!  ")
        cnab['contract_rate'] = contract['contract_rate'].replace(".", "").zfill(6)
        cnab['codigoIndexador'] = "00"
        cnab['percentIndexer'] = "0000860000"
    ...
    cnab = file[
        [
            'identificacaoArqRemessa', 'DocType', 'contract', 'CS', 'installment_number', 'installments',
            'operation_date',
            'date', 'amount_total',
    ...
    fline = "01{0}{0}0xxxxxx  08732000".format(
        today).ljust(543)
    ...
    filename_cnab550 = "xxx_{}_0001.rem".format(today)
    filename_cnab551 = "xxxx_{}_0001.rem".format(today)
    filename_csv = "xxx_{}_0001.txt".format(today)
    del cnab
    try:
        os.system("rm -rf /tmp/{}".format(filename_cnab550))
        os.system("rm -rf /tmp/{}".format(filename_cnab551))
        os.system("rm -rf /tmp/{}".format(filename_csv))
    except:
        pass

    os.mknod("/tmp/" + filename_cnab550)
    os.mknod("/tmp/" + filename_cnab551)
    os.mknod("/tmp/" + filename_csv)

    with open("/tmp/" + filename_cnab550, 'wb') as cm550:
        cm550.write(bytes(cnab550, 'utf8'))
        cm550.close()

    with open("/tmp/" + filename_cnab551, 'wb') as cm551:
        cm551.write(bytes(cnab551, 'utf8'))
        cm551.close()

    with open("/tmp/" + filename_csv, 'wb') as cm550csv:
        cm550csv.write(bytes(csv, 'utf8'))
        cm550csv.close()

    s3client.upload_file("/tmp/" + filename_cnab550, "x", "cnab550/" + filename_cnab550)
    s3client.upload_file("/tmp/" + filename_cnab551, "x", "cnab550/" + filename_cnab551)
    s3client.upload_file("/tmp/" + filename_csv, "x", "cnab550/" + filename_csv)
